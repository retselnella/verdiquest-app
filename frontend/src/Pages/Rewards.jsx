...
const Rewards = () => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/admin/rewards/${productId}`,
        {
          method: "PUT",
          headers: {
    ...
    try {
      ...
      if (response.ok) {
        const updatedRewards = rewards.filter(
          (reward) => reward.ProductId !== productId
        );
  ...
  const fetchRewardsData = async () => {
    try {
      ...
      const sortedRewards = data.sort((a, b) => a.OrganizationId - b.OrganizationId);
  ...
  const handleAddProduct = async (e) => {
    ...
    // Check if the productName already exists in the rewards array
    const isProductNameExists = rewards.some(
      (reward) => reward.ProductName === productName
    );
      ...
      <div>
              ...
              <Form onSubmit={handleAddProduct}>
                ...
                <Form.Group controlId="formPointsRequired">
                  ...
                  <Form.Control
                    ...
                    onChange={handlePointsRequiredChange}
                  />
                </Form.Group>
                <br />
                <Button
        ...
        <div className="table-reward-container">
          <div className="table-reward-responsive">
            <h3 style={{ textAlign: "center" }}>Rewards View</h3>
            <Button
              variant="primary"
              ...
              <tbody>
                {rewards.map((reward, index) => (
                  <tr key={reward.ProductId}>
                    <td>{reward.ProductId}</td>
                    <td>{reward.OrganizationName}</td>
          ...
          >
            ...
            <Modal.Footer>
              ...
              <Button
                variant="danger"
                onClick={() => handleDeleteReward(productToDelete.ProductId)}
              >
                Delete
              </Button>
            </Modal.Footer>

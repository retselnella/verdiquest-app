...
import PieChart from '../components/PieChartGender';
const Gender = () => {
    {
        ...
        const fetchData = async () => {
            try {
                const response = await axios.get("http://localhost:3001/admin/gender");
                const labels = response.data.map(x => x.Gender); // Age ranges
                const totalUsers = response.data.map(x => x.Total   ); // User counts
